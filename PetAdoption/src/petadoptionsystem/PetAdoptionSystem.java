...
public class PetAdoptionSystem {
    ...
    static String inputPetAge() {
        while (true) {
            ...
            //ensure age contains only 1 or 2 digits
            if (!validateInput(age, "\\d{1,2}")) {
                System.out.println("\nInvalid Age Format (Age Must Not Exceed 2 Digits)\n");
                continue;
            }

            //convert entered age into integer for validation
    ...
    static String updatePetName(String currentPetName) {
        while (true) {
            ...
            if (!validateInput(newPetName, "[A-Za-z\\s]+")) {
    ...
    static boolean validatePetName(String petName) {
        return validateInput(petName, "[A-Za-z.,\\s]+");
    }
...
//in view all users, admin can remove a staff (requies re entering password)
//first ask (y/n) if want to remove a staff, Enter ID to remove: cannot remove admins
//regex:
//make an alternate version with registration, (userInput logics)
//users id's are automatic, but not when adding pets?
